...
$include (F120.inc)
;EFX_A		equ	P1.2		; PWM für EFX_A
;EFX_B		equ	P1.3		; PWM für EFX_B
;EFX_C		equ	P1.4		; PWM für EFX_C
YELLOW_LED	equ	P1.5		; PWM für gelbe LED
GREEN_LED	equ	P1.6            ; PWM für grüne LED
RED_LED		equ	P1.7		; PWM für rote LED
...
EFX_Select	equ	P3		; P3.0 - P3.6 für EFX Select
Modul1_Play	equ	P3.7		; P3.7 zeigt Modul2 an, wenn alle Voices des Modul1 belegt sind
...
;*************************************************************************************************************
	...
	MIDI_CC_Changed	BIT	27h.6	;=1 wenn über MIDI CC Daten angekommen sind
	...
	MidiCLKflag_DCO1M BIT	29h.3	;Bit wird gesetzt bei empfangener Midi Clock ($F8)
	MidiCLKflag_DCO2M BIT	29h.4	;Bit wird gesetzt bei empfangener Midi Clock ($F8)
	...
	COPY_B1_EN	BIT	2dh.7	;=1 wenn Bank1 nach extFlash2 0-31 kopiert werden soll
...
START:	MOV 	WDTCN, #0DEh
	...
	clr	COPY_SD_EN
	clr 	COPY_B1_EN		;=1 wenn Bank1 nach extFlash2 0-31 kopiert werden soll
...
Loop:			
	mov	r0,#EFXLFO1TAR
	movx	a,@r0
	cjne	a,#0,no_EFXLFO1TAR_0	;wenn 0 dann tue nichts
	jmp	no_EFXLFO1TAR_raus
no_EFXLFO1TAR_0:
	cjne	a,#1,no_EFXLFO1TAR_1
	MOV	R0,#EFX_A		;wenn 1 dann Ziel EFX_A			
	MOVX	a,@R0
	mov	b,a
	mov	r0,#EFXLFO1VAL
	movx	a,@r0
	...
	jmp	no_EFXLFO1TAR_raus
no_EFXLFO1TAR_1:
	cjne	a,#2,no_EFXLFO1TAR_2
	MOV	R0,#EFX_B		;wenn 2 dann Ziel EFX_B			
	MOVX	a,@R0
	mov	b,a
	mov	r0,#EFXLFO1VAL
	movx	a,@r0
	...
	jmp	no_EFXLFO1TAR_raus
no_EFXLFO1TAR_2:
	cjne	a,#3,no_EFXLFO1TAR_raus
	MOV	R0,#EFX_C		;wenn 3 dann Ziel EFX_C			
	MOVX	a,@R0
	mov	b,a
	mov	r0,#EFXLFO1VAL
	movx	a,@r0
...
no_EFXLFO1TAR_raus:
	...
	mov	r0,#EFXLFO2TAR
	movx	a,@r0
	cjne	a,#0,no_EFXLFO2TAR_0	;wenn 0 dann tue nichts
	jmp	no_EFXLFO2TAR_raus
no_EFXLFO2TAR_0:
	cjne	a,#1,no_EFXLFO2TAR_1
	MOV	R0,#EFX_A				
	MOVX	a,@R0
	mov	b,a
	mov	r0,#EFXLFO2VAL
...
no_EFX_Send_Changed:
	jnb	EFX_Return_Changed,no_EFX_Return_Changed
	...
	mov	COMMAND,#050h			;I2C Adresse DS1803 POT0 und POT1
	mov	HIGH_ADD,#0AFh			;I2C Command DS1803 POT0 und POT1
	mov	LOW_ADD,#0AFh			;I2C Command DS1803 POT0 und POT1
	MOV	r0,#EFXRETURN
	movx	a,@r0
	rl	a				;0-254
	mov	WORD,a				;Data POT0 und POT1
	clr	STO
	setb	STA
	clr	EFX_Return_Changed
	jmp	no_EFX_Return_Changed

no_Volume_LFO1_Changed_hlp:
	ljmp	no_Volume_LFO1_Changed
no_EFX_Return_Changed:
...
no_Volume_LFO2_Changed:
	...
	clr	ET0				;Timer 0 IRQ DIS
	clr	ET1				;Timer 1 IRQ DIS
	clr	ET2				;Timer 2 IRQ DIS
	clr	ET3				;Timer 3 IRQ DIS
	...
	nop
	nop
	mov     EIE1,#0h			;SPI und I2C DIS
	;clr	ESMB0				;I2C IRQ DIS
	call	XRAM2MIDI			;Sende XRAM 02h-53h an MIDI OUT
	;setb	ESMB0
	mov  	EIE1,#003h			;SPI und I2C EN
	setb	ET0
	setb	ET1
	setb	ET2
	setb	ET3
...
no_SAVE:
	...
	clr	COPY_EN
no_COPY:
	jnb	MIDI_CC_Changed,no_MIDI_CC_Changed
	clr	MIDI_CC_Changed
