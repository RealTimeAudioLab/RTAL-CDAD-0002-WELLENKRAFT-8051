...
Timer_Init:
	...
	mov  TH0,	#000h		; 11.059.200 Hz / (256 - X) = 43200 Hz für LFO
    	...
    	mov  SFRPAGE,   #TMR2_PAGE
    	mov  TMR2CF,    #008h
    	mov  RCAP2L,    #09eh		; 49928 Hz Abtastrate für VOICE2
    	mov  RCAP2H,    #0f5h

	mov  SFRPAGE,	#TMR3_PAGE;	; Timer 3
    	mov  TMR3CN,	#04h
    	mov  TMR3CF,    #08h		; Sysclk
	mov  RCAP3L,	#09eh		; 49928 Hz Abtastrate für VOICE1
    	mov  RCAP3H,	#0f5h		; 
...
Port_IO_Init:
        ...
        mov  P3MDOUT,   #01111111b	;P3.0 bis 3.6 = Outputs, P3.7 = Input
	mov  P3,	#10000000b
	;mov  XBR0,	#037h		; mit PWM
...
Interrupts_Init_2:
    	mov  IE,        #0B2h		; EN All IRQs, UART0, T0, T2
    	mov  IP,        #030h		; Prio UART0, T2
   	mov  EIE1,      #00Bh		; EN SPI und I2C und PCA
    	mov  EIE2,      #041h		; EN UART1 und T3
    	mov  EIP2,      #041h		; Prio UART1 und T3
    	ret
