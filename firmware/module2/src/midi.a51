...
MIDI_Sysex_Data_2:
	...
	jmp	irq_end
MIDI_Sysex_Data_3:
	cjne	a,#02h,MIDI_Sysex_Data_4				;02h für Receive CC en/dis
...
MIDI_Sysex_Data_22_1:
	...
	cjne	a,#30h,MIDI_Sysex_Data_22_2				;Multi Mode?	
	mov	r1,#CHANNEL						;wenn ja dann
	movx	a,@r1
	anl	a,#00001111b
	inc	a							;Midi Channel +1 
	anl	a,#00001111b
	movx	@r1,a		
	jmp	MIDI_Sysex_Data_22_3

MIDI_Sysex_Data_22_2:														
	mov	r1,#CHANNEL
	movx	a,@r1
	anl	a,#11110000b	
	cjne	a,#30h,MIDI_Sysex_Data_22_3				;War vorher Multi Mode?	
	mov	r1,#CHANNEL						;wenn ja dann
	movx	a,@r1
	anl	a,#00001111b
	dec	a							;Midi Channel -1 
	anl	a,#00001111b
	movx	@r1,a	
	
MIDI_Sysex_Data_22_3:
	mov	r1,#CHANNEL
	movx	a,@r1
	anl	a,#00001111b
...
Note_OFF:
	;jnb	Modul1_Play,Note_OFF_end	;wenn =0h dann Voices auf Modul1 frei			
						;wenn =1h dann alle Voices auf Modul1 belegt
	mov	voice_pos,#01h			;Note Off für Voices 5-8 einleiten
	mov	r1,#Taste1
Note_OFF_Such:
	mov	a,@r1
...
Note_ON:					;Note On für Voices 5-8 einleiten
	mov	r1,#CHANNEL
	movx	a,@r1
	anl	a,#11110000b
	jnz	Note_ON_1
...
Note_ON_1:
	...
	mov	a,#64h-36			;Taste F5
	cjne	a,midibyte1,$+3
	jnc	Note_OFF_end			;wenn gedrückte Taste kleiner als Splitpunkt F5 dann raus	
Note_ON_no_Split:			
	SETB	GREEN_LED			;wenn =1h dann alle Voices auf Modul1 belegt
...
no_LFO2_KEYB_SYNC:
	...
	mov	voice_mem,#00
	mov	voice_pos,#0FEh			;note on für Voices 5-8 einleiten
	mov	r1,#Taste1
Note_ON_Such:
	mov	a,@r1
	cjne	a,midibyte1,Note_ON_Weiter
	mov	a,voice_ds			
	anl	a,voice_pos
...
Note_ON_Clear:		
	...
	movx	a,@r1
	anl	a,#11110000b
	jnz	Note_ON_Clear_1			;wenn 0 dann Poly Mode sonst weitere Modes
	jmp	irq_end
Note_ON_Clear_1:
	cpl	voice23
...
Weiter_B0:
	CJNE	A,#0B0h,Weiter_E0_hlp		;Wenn kein B0h dann E0?
...
b0_01_2:
	cjne	a,#02,b0_01_3
	MOV	R1,#DCFRESO			;DCF Res
	MOV	A,midibyte2
	rl	a								
	MOVX	@r1,A
	jmp	irq_end
b0_01_3:
	cjne	a,#03,b0_01_4			;DCO1/2 LFO1
	MOV	R1,#DCO1LFO1
	MOV	A,midibyte2				
	rl	a					
	MOVX	@R1,A
...
b0_01_4:
	...
	jmp	irq_end
b0_01_5:
	cjne	a,#05,b0_01_6			;DCO1 WAVE
	MOV	A,midibyte2
	MOV	R1,#DCO1WAVE
	MOVX	@R1,A
	jmp	irq_end
b0_01_6:
	cjne	a,#06,b0_01_7			;DCO2 WAVE
	MOV	A,midibyte2
	MOV	R1,#DCO2WAVE
	MOVX	@R1,A
	jmp	irq_end
b0_01_7:					;DCO1 und DCO2
	cjne	a,#07,b0_01_8
	MOV	A,midibyte2
	MOV	R1,#DCO1WAVE
	MOVX	@R1,A
...
b0_01_8:
	cjne	a,#8,b0_01_9
	MOV	A,midibyte2
	mov	r1,#DCO1_WAVE_INC		;DCO1 WAVE INC-Wert
	movx	@r1,a
	jmp	irq_end
b0_01_9:
	cjne	a,#9,b0_01_10
	MOV	A,midibyte2
	mov	r1,#DCO2_WAVE_INC		;DCO2 WAVE INC-Wert
	movx	@r1,a
	jmp	irq_end
b0_01_10:
	cjne	a,#10,b0_01_11
...
b0_no_wheel:
	jnb	rcv_midi_cc,irq_end_h		;Wenn Flag = 0 dann kein Midi CC Empfang
